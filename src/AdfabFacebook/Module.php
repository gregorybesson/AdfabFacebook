...
class Module
{
    ...
    {
        $game       = $e->getParam('game');
        $appService = $e->getTarget()->getServiceManager()->get('adfabfacebook_app_service');
    ...
    public function getServiceConfig()
    {
        return array(
            'aliases' => array(
                'adfabfacebook_doctrine_em' => 'doctrine.entitymanager.orm_default',
            ),

            'invokables' => array(
                'adfabfacebook_app_service' => 'AdfabFacebook\Service\App',
            ),

            'factories' => array(
                'adfabfacebook_module_options' => function ($sm) {
                    $config = $sm->get('Configuration');

                    return new Options\ModuleOptions(isset($config['adfabfacebook']) ? $config['adfabfacebook'] : array());
                },
                'adfabfacebook_app_mapper' => function ($sm) {
                    return new \AdfabFacebook\Mapper\App(
                        $sm->get('adfabfacebook_doctrine_em'),
                        $sm->get('adfabfacebook_module_options')
                    );
                },
                'adfabfacebook_app_form' => function($sm) {
                    $translator = $sm->get('translator');
                    $options = $sm->get('adfabfacebook_module_options');
